...
load('results/Final_results/2019417-1057_circle_R1_t180_l0156.mat')
    T       = 180;         % vid vilken tidpunkt plottar vi resultatet mot kiralitet
    T2      = 180;         % Hur l̴ng tid plottar vi arean 
...
for i = 1:size(meanArea,2)
    num = round(binKir(i),3,'significant');
    txt = ['$\omega$ = ',num2str(num),' rad/s'];
    P(i) = plot((1:size(meanArea,1))/2, meanArea(:,i)/maxArea,'color',c(i,:),'DisplayName',txt,'Linewidth', 4)
end
...
xlabel('Tid (s)', 'Interpreter', 'latex', 'fontsize', 50)
